...
interface LegalCenterWelcomeModalProps {
  isOpen: boolean;
  onAccept: () => void;
  onReject: () => void;
  onNeverShow: () => void;
 ...
 */
const LegalCenterWelcomeModal: React.FC<LegalCenterWelcomeModalProps> = ({ isOpen, onAccept, onReject, onNeverShow }) => {
  if (!isOpen) return null;

  // Estado para la selección de la guía, null hasta que el usuario elija.
  const [guideSelection, setGuideSelection] = useState<'yes' | 'no' | null>(null);
  ...
  return (
    <div className="fixed inset-0 bg-white md:bg-black md:bg-opacity-60 flex items-center justify-center z-[100] p-0 md:p-4">
      <div className="bg-white rounded-none md:rounded-2xl max-w-sm w-full h-full md:h-auto shadow-xl animate-fadeScaleIn p-5 overflow-y-auto">
