...
import { Zap, Shield, ExternalLink, FileText } from 'lucide-react';
import IntegrationModal from './IntegrationModal';
import { requestMifielIntegration } from '../utils/integrationUtils';
import SignatureWorkshop from './SignatureWorkshop';
import type { IntegrationData } from './IntegrationModal';

interface LegalProtectionOptionsProps {
...
type SelectedIntegration = 'signnow' | null;

const LegalProtectionOptions: React.FC<LegalProtectionOptionsProps> = ({ documentId, documentHash, userId, originalFile, documentName }) => {
  ...
  const handleMifielClick = async () => {
    if (!documentId || !documentHash || !userId) {
      toast.error('Falta información del documento para solicitar certificado NOM-151.');
      return;
    }
    try {
      const result = await requestMifielIntegration(documentId, 'nom-151', documentHash, userId);
      setModalData(result as IntegrationData);
      setIsModalOpen(true);
    } catch (error) {
  ...
  const handleConfirm = async (paymentData: IntegrationData & { paymentMethod?: string; status?: string }) => {
