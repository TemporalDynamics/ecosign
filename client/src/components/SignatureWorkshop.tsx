...
const SignatureWorkshop: React.FC<SignatureWorkshopProps> = ({
  originalFile,
  documentId = null,
  documentHash = null,
  userId = null,
  ...
  const [signNowError, setSignNowError] = useState<string | null>(null);
  ...
  const dragStartRef = useRef<DragState>(null);
  ...
  const getCanvasPoint = (event: TouchEvent | MouseEvent) => {
    const canvas = canvasRef.current;
    if (!canvas) return null;
  ...
  const handleCanvasStart = (event: TouchEvent | MouseEvent) => {
  ...
  const handleCanvasMove = (event: TouchEvent | MouseEvent) => {
    if (!isDrawing) return;
    event.preventDefault();
    const ctx = getCanvasContext();
    const point = getCanvasPoint(event);
  ...
  const handleSignatureMouseDown = (e: React.MouseEvent<HTMLDivElement>) => {
    handleSignaturePointerDown(e.nativeEvent, false);
  };
  ...
  const handleResizeMouseDown = (e: React.MouseEvent<HTMLDivElement>) => {
    e.stopPropagation();
    handleSignaturePointerDown(e.nativeEvent, true);
  };
  ...
  const handleSignatureMove = (event: MouseEvent | TouchEvent) => {
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    try {
      const signatureImg = signatureImage;
      if (!signatureImg) {
        setSignNowError('Primero debes guardar tu firma (paso 1).');
        return;
      }
      ...
      const errorMessage = error instanceof Error ? error.message : String(error) || 'Error conectando con SignNow';
      ...
      <div className="grid md:grid-cols-2 gap-4">
        <div className="border border-gray-200 rounded-lg p-3">
          ...
          {!hasSavedSignature && (
            ...
            onMouseDown={(e) => handleCanvasStart(e.nativeEvent)}
            onMouseMove={(e) => handleCanvasMove(e.nativeEvent)}
            onMouseUp={handleCanvasEnd}
            onMouseLeave={handleCanvasEnd}
            onTouchStart={(e) => handleCanvasStart(e.nativeEvent)}
            onTouchMove={(e) => handleCanvasMove(e.nativeEvent)}
