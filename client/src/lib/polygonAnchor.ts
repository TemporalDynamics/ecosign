...
export async function anchorToPolygon(documentHash: string, options: AnchorRequestOptions = {}): Promise<AnchorResponse> {
  ...
  try {
    ...
    return {
      success: false,
      error: error instanceof Error ? error.message : 'Unknown error during Polygon anchoring'
    };
  }
}
 ...
 */
export async function verifyPolygonAnchor(documentHash: string): Promise<{
  anchored: boolean;
  message?: string;
  status?: string;
  anchorId?: string;
 ...
 * @returns {Promise<Object>} Estado del anchor
 */
export async function getAnchorStatus(anchorId: string): Promise<{
  id: string;
  status: string;
  type: string;
  documentHash: string;
  ...
  try {
    ...
    throw error instanceof Error ? error : new Error('Unknown error getting anchor status');
  }
}
 ...
 */
export async function listUserAnchors(userId: string, options: AnchorRequestOptions = {}): Promise<
  Array<{
    id: string;
    documentHash: string;
    status: string;
...
> {
  ...
  try {
    ...
    return (data || []).map((anchor: any) => ({
      ...
      explorerUrl:
        anchor.metadata?.txHash && anchor.anchor_type === 'polygon'
