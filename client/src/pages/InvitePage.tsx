...
export default function InvitePage() {
  ...
  const [invite, setInvite] = useState<InviteAccess | null>(null);
  const [document, setDocument] = useState<InviteDocument | null>(null);
  ...
  async function checkAuthAndInvite() {
    try {
      ...
      // Access granted!
      setDocument(accessData.access.document as InviteDocument);
      setInvite(accessData.access as InviteAccess);
      setNdaAccepted(true);
      setLoading(false);
  ...
  if (document && invite) {
    ...
    const signerLinkId = inviteMeta?.signerLinkId;
    return (
        ...
        <div className="max-w-4xl mx-auto py-12 px-4 sm:px-6 lg:px-8">
          ...
          <div className="bg-white rounded-lg shadow-lg p-8">
            ...
            <div className="flex gap-4">
              ...
              {invite.role === 'signer' && signerLinkId && (
                <button
                  onClick={() => navigate(`/sign/${signerLinkId}`)}
          ...
          {inviteMeta && (
            <div className="bg-white rounded-lg shadow-lg p-6 mt-6">
              ...
              <div className="text-xs text-gray-600 space-y-1">
                <p>NDA aceptado: {inviteMeta.ndaAcceptedAt ? new Date(inviteMeta.ndaAcceptedAt).toLocaleString('es-AR') : '—'}</p>
                <p>Acceso aceptado: {inviteMeta.acceptedAt ? new Date(inviteMeta.acceptedAt).toLocaleString('es-AR') : '—'}</p>
                <p>Expira: {inviteMeta.expiresAt ? new Date(inviteMeta.expiresAt).toLocaleString('es-AR') : '—'}</p>
              </div>
            </div>
          )}
        </div>
