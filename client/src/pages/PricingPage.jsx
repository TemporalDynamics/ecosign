          ...
          <div className="flex justify-between h-16 items-center">
            <div className="flex items-center space-x-4">
              <Link to="/" className="flex items-center space-x-3">
                <span className="text-2xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-black to-gray-800">EcoSign</span>
              </Link>
              <Link
                to="/dashboard"
            ...
            <div className="flex items-center space-x-6">
              ...
              <Link
                to="/login"
                className="bg-gradient-to-r from-black to-gray-800 hover:bg-gray-800  text-white font-semibold px-6 py-2.5 rounded-lg transition duration-300 shadow-md hover:shadow-lg"
              >
                Comenzar Gratis
              </Link>
            </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-24 pt-32">
        ...
        <div className="grid md:grid-cols-3 gap-8 mb-16">
          {plans.map((plan, index) => (
            <div key={index} className={`relative bg-white rounded-2xl shadow-xl overflow-hidden border-2 ${plan.popular ? 'border-black500' : 'border-gray-200'}`}>
              {plan.popular && (
                <div className="absolute top-0 left-1/2 transform -translate-x-1/2 -translate-y-1/2 bg-gradient-to-r from-black to-gray-800 text-white text-xs font-bold px-4 py-1 rounded-full">
                  MÁS POPULAR
                </div>
              )}
              <div className="p-8">
                <h2 className="text-2xl font-bold text-gray-900 mb-2">{plan.name}</h2>
                <div className="text-4xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-black to-gray-800 mb-2">{plan.price}</div>
                <p className="text-gray-600 mb-6">{plan.description}</p>

                <ul className="space-y-4 mb-8">
                  {plan.features.map((feature, idx) => (
                    <li key={idx} className="flex items-start">
                      <Check className="w-5 h-5 text-black mr-2 flex-shrink-0 mt-0.5" strokeWidth={2.5} />
                ...
                <Link to="/login" className={`block w-full py-3 px-6 rounded-lg font-bold text-center transition duration-300 ${
                  plan.popular
                    ? 'bg-gradient-to-r from-black to-gray-800 hover:bg-gray-800  text-white shadow-lg'
        ...
        <div className="bg-gray-100 p-8 md:p-12 rounded-2xl border border-black200 mb-12">
        ...
        <div className="text-center text-gray-600 mb-8">
          <p>¿Tienes preguntas? <a href="mailto:contact@verifysign.com" className="text-black hover:text-cyan-700 font-semibold">Contáctanos</a> para una demostración personalizada.</p>
        </div>
      </div>
