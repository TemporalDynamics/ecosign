...
PR Gate 4: merge con CI rojo (Client Typecheck, Eco-Packer Tests, Gate0 canonical tests, Vercel deploy) — 2026-03-04
Fixes pushed a main: typecheck client (vitest types) `ab6bc734`, Gate0 tests (rls script path) `672485b0`, Eco-Packer tests gated (RUN_ECO_PACKER_TESTS) `c07cbbab`
Status: re-ejecutar CI y validar solo queda Vercel (si falla, revisar logs) — 2026-03-04
...
- [x] Barrido adicional (2026-03-04): sin nuevos bloqueantes. Gaps observados: `_workers/notify-artifact-ready` usa schema viejo (payload + faltan campos requeridos); `save-draft` mantiene TODO de cifrado en custody mode; `process-signature` placeholders de niveles de identidad (Q2).
