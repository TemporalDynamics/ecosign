## Iteración: Arranque P0 Pre-Release (Roadmap Infalible) — 2026-02-14

### 🎯 Resumen
Se inicia formalmente la fase P0 (bloqueantes pre-release) con foco en riesgos
de claim, confiabilidad de anclajes y estabilidad operativa de notificaciones.
La arquitectura canónica se considera estable; el trabajo pendiente es
operacional/comunicacional.

### ✅ Decisiones P0 (con criterio de cierre)
- **Anchoring + Rekor en estado explícito (sin ambigüedad):**
  - Rekor debe quedar en uno de dos estados: **ON visible** o **OFF no mencionado**.
  - Cierre: estado definido en producto + documentación de operación.
- **Claims legales/copy pública alineados a evidencia:**
  - Evitar claims absolutos ("garantía definitiva", etc.).
  - Usar lenguaje de evidencia verificable, trazabilidad e integridad.
  - Cierre: revisión de copy pública principal y plantillas críticas.
- **Confiabilidad de mails terminales (entrega > diseño):**
  - El incidente actual queda clasificado como operativo (429 provider rate limit).
  - Cierre: estrategia de retry/backoff + dedupe de terminales + monitoreo de cola.
- **Safety net operativo explícito:**
  - Definir modo degradado/freeze manual + runbook mínimo.
  - Cierre: procedimiento documentado y ejecutable por operador.

### 🧱 Alcance por prioridad
- **P0 (bloqueante):** Anchoring/Rekor, claims legales, confiabilidad mail, safety net.
- **P1 (importante):** templates marketineros, signer→cuenta+custody, UX cleanup.
- **P2 (diferenciación):** verificador narrativo/timeline y mejoras de generalización UX.

### 📌 Estado de partida
- Invariantes EPI: verde.
- Canary técnico en producción: activo.
- Riesgo actual dominante: operación/UX/comunicación (no integridad canónica).

### 🔜 Próximo corte inmediato
1. Congelar estado **Rekor ON/OFF** y reflejarlo en producto + docs.
2. Definir checklist mínimo de copy legal-safe en páginas/mails públicos.
3. Definir plan de hardening de notificaciones (post-canary para implementación,
   pre-canary para documentación y monitoreo).

### ✅ Cierre P0-1 (estado Rekor) — 2026-02-14
- **Decisión:** `Rekor = OFF` para pre-release/canary actual.
- **Rationale operativo/legal:**
  - El core de evidencia ya está cubierto por ECO + TSA + anchors canónicos.
  - Evitamos claim ambiguo de transparency log sin runbook operativo dedicado.
  - Reducimos riesgo de percepción/legal hasta tener operación explícita end-to-end.
- **Regla de producto/copy (vigente):**
  - Rekor **no se menciona** en superficies públicas/canónicas de producto.
  - No se usa como claim comercial ni legal en esta etapa.
- **Condición para pasar a ON (futuro):**
  1. Secret/config de firma habilitado y validado en entorno productivo.
 2. Métricas + alertas de éxito/fallo/timeouts de publicación.
 3. Runbook operativo de incidentes específico para Rekor.
 4. Mensajería de producto actualizada con wording legal-safe.
