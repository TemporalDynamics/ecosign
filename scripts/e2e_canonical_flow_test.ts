...
async function runEndToEndTest(): Promise<boolean> {
  ...
  // 1. Crear un nuevo document_entity para la prueba
  ...
  try {
    const { data: jobs, error: jobsError } = await supabase
      ...
      .eq('entity_id', entity.id)  // Usar el ID de la entidad creada
  ...
  try {
    ...
    if (!runs || runs.length === 0) {
      console.log('   ⚠️  No se encontraron ejecuciones por ExecutionEngine (puede ser normal si no se activó)');
      results.push({
    ...
    console.log(`❌ Error verificando ejecuciones: ${error.message}\n`);
    results.push({
      step: 'execution_engine_processed_jobs',
      success: false,
      details: { error: error.message },
  ...
  const overallSuccess = successCount >= 2; // Al menos creación y jobs
  const successEmoji = overallSuccess ? '✅' : '❌';

  console.log(`\n${successEmoji} RESULTADO FINAL DEL FLUJO END-TO-END: ${overallSuccess ? 'EXITOSO' : 'CON PROBLEMAS'}`);
  console.log(`   Pasos exitosos: ${successCount}/${results.length}`);

  if (overallSuccess) {
    console.log('\n🎯 FLUJO CANÓNICO VERIFICADO:');
    console.log('   - DecisionAuthority procesa eventos y crea jobs');
    console.log('   - ExecutionEngine puede procesar jobs (cuando se activa)');
    console.log('   - Todo registrado como eventos canónicos');
    console.log('   - Separación de responsabilidades mantenida');
  } else {
    console.log('\n⚠️  FLUJO CON PROBLEMAS:');
