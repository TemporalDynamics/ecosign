...
echo "runtime cron verification at $(date -u +"%Y-%m-%dT%H:%M:%SZ")"
echo

echo "current runtime cron status:"
psql "$DB_URL" -v ON_ERROR_STOP=1 -P pager=off -c \
  "SELECT * FROM public.get_cron_runtime_status();"
...
WITH inv AS (SELECT * FROM public.get_cron_runtime_status()),
checks AS (
...
echo
if [ "$FAILS" = "0" ]; then
  echo "RESULT: PASS (runtime cron status matches canonical expectations)"
  exit 0
fi

echo "RESULT: FAIL ($FAILS runtime cron checks failed)"
exit 1
