...
serve(async (req) => {
  ...
  try {
    ...
    if (!signer.email) {
      return json({ error: 'Signer email missing', details: 'signer_email_empty' }, 400, corsHeaders);
    }

    if (signer.status !== 'signed') {
      return json({ error: 'Signer is not signed' }, 403, corsHeaders);
    }

    if (!signer.workflow?.id) {
      return json({ error: 'Workflow not found for signer', details: 'workflow_missing' }, 500, corsHeaders);
    }

    if (signer.workflow.status && !['active', 'completed'].includes(signer.workflow.status)) {
      return json({ error: `Workflow not available (status=${signer.workflow.status})` }, 403, corsHeaders);
    }
    ...
    if (existingOtpError) {
      console.warn('send-signer-recovery-otp existing record check failed', existingOtpError);
    ...
    if (upsertErr) {
      console.error('send-signer-recovery-otp upsert failed', upsertErr);
      return json({ error: 'Could not generate OTP', details: upsertErr.message }, 500, corsHeaders);
    }
    ...
    await appendCanonicalEvent(
      supabase,
      {
        ...
        workflow_id: signer.workflow.id,
        signer_id: signer.id,
        payload: { expires_at: expiresAt, reason: 'recovery' }
      },
    ...
    console.error('send-signer-recovery-otp error', error);
