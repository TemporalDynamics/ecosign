...
serve(async (req) => {
  ...
  const auth = await requireInternalAuthLogged(req, 'submit-anchor-polygon', { allowCronSecret: true });
  if (!auth.ok) {
  ...
  if (correlationId && correlationId !== documentEntityId) {
    console.warn('[submit-anchor-polygon] correlation_id mismatch - overriding to canonical', {
