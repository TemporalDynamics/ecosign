...
-- Fecha: 2025-11-27
-- Descripción: Infraestructura forense ECOX + Notificaciones al creador
-- ============================================

-- ============================================
-- PARTE 1: TABLA ECOX AUDIT TRAIL (Trazabilidad Forense)
-- ============================================
-- Esta tabla es el corazón de la evidencia forense de EcoSign
...
CREATE TABLE IF NOT EXISTS public.ecox_audit_trail (
    ...
    event_type TEXT NOT NULL CHECK (event_type IN (
        'access_link_opened',        -- El firmante abrió el link de firma
        'nda_accepted',              -- Aceptó el NDA
        'mfa_challenged',            -- Se le presentó el desafío MFA
        'mfa_success',               -- MFA completado exitosamente
        'mfa_failed',                -- MFA falló
        'document_decrypted',        -- Documento descifrado y mostrado
        'document_viewed',           -- Firmante vio el documento
        'signature_started',         -- Empezó el proceso de firma (dibujando/escribiendo)
        'signature_applied',         -- Firma aplicada al documento
        'signature_completed',       -- Proceso de firma completado
        'eco_downloaded'             -- Descargó el certificado .ECO
    )),
    ...
    -- Detalles Específicos del Evento (Flexible)
    details JSONB,                   -- Información adicional específica por evento
...
-- Comentarios
COMMENT ON TABLE public.ecox_audit_trail IS 'Registro forense completo de cada acción del firmante - Base del certificado ECOX';
COMMENT ON COLUMN public.ecox_audit_trail.event_type IS 'Tipo de evento en el flujo de firma';
COMMENT ON COLUMN public.ecox_audit_trail.details IS 'JSON con datos específicos: { "mfa_method": "TOTP", "time_to_decrypt_ms": 200, "signature_coords": {...} }';
COMMENT ON COLUMN public.ecox_audit_trail.document_hash_snapshot IS 'Hash del documento en el momento del evento (para verificación temporal)';
...
-- Policy: La aplicación (service_role) puede insertar eventos
...
-- ============================================
-- PARTE 3: FUNCIÓN PARA REGISTRAR EVENTOS ECOX
-- ============================================
-- Función auxiliar que la aplicación puede llamar desde Edge Functions
...
COMMENT ON FUNCTION public.log_ecox_event IS 'Registra un evento en el audit trail ECOX desde la aplicación';
...
COMMENT ON FUNCTION notify_creator_on_signature IS 'Notifica al creador cuando un firmante completa su firma con detalles técnicos';

-- ============================================
-- PARTE 5: FUNCIÓN PARA GENERAR ECOX COMPLETO
-- ============================================
-- Esta función genera el contenido completo del certificado ECOX
