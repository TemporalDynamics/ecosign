-- ============================================
-- Fix D5 Shadow Mode: Log ALL comparisons + Fix dedupe mismatch
-- ============================================
-- Problemas identificados:
-- 1. Shadow mode solo logueaba divergencias, no matches
-- 2. existing_count quedaba NULL si legacy_decision = false desde el inicio
-- 3. Legacy dedupe usa (workflow_id, email, type) pero canonical usaba (workflow_id, signer_id, type)
--
-- Fixes aplicados:
-- 1. Loguear TODAS las comparaciones (matches + divergencias)
-- 2. Inicializar existing_count := 0
-- 3. Alinear dedupe: ambos usan (workflow_id, email, type)
-- ============================================

-- ============================================
-- 1. Fix canonical dedupe (alinear con legacy)
-- ============================================
-- Drop old version first (4 params)
DROP FUNCTION IF EXISTS should_notify_signer_link_canonical(TEXT, TEXT, UUID, UUID);

-- Create new version (5 params)
CREATE OR REPLACE FUNCTION should_notify_signer_link_canonical(
  p_operation TEXT,
  p_signer_status TEXT,
  p_signer_id UUID,
  p_workflow_id UUID,
  p_signer_email TEXT  -- ✅ AGREGADO: Para usar mismo dedupe que legacy
) RETURNS BOOLEAN AS $$
DECLARE
...
BEGIN
  ...
  -- ✅ FIX: Usar MISMA dedupe key que legacy (email-based, no signer_id-based)
  SELECT EXISTS (
    SELECT 1
    FROM workflow_notifications
    WHERE workflow_id = p_workflow_id
      AND recipient_email = p_signer_email  -- ✅ Mismo que legacy
...
COMMENT ON FUNCTION should_notify_signer_link_canonical IS
  'D5 shadow (FIXED): Usa misma dedupe key que legacy (email-based)';

-- ============================================
-- 2. Fix trigger: Log ALL comparisons
-- ============================================
...
DECLARE
  ...
  owner_email TEXT;
  sign_link TEXT;
  existing_count INT := 0;  -- ✅ FIX: Inicializar en 0
...
BEGIN
  ...
  v_canonical_decision := should_notify_signer_link_canonical(
    ...
    NEW.id,
    NEW.workflow_id,
    NEW.email  -- ✅ FIX: Pasar email para dedupe consistente
  );

  -- ============================================
  -- SHADOW COMPARISON - ALWAYS LOG (FIXED)
  -- ============================================
  INSERT INTO shadow_decision_logs (
    ...
    v_legacy_decision != v_canonical_decision,
    jsonb_build_object(
      'operation', v_operation,
      'signer_status', NEW.status,
      'signer_email', NEW.email,
      'existing_count', existing_count,  -- ✅ Ya no puede ser NULL
...
COMMENT ON FUNCTION notify_signer_link IS
  'D5 shadow mode (FIXED): Logs ALL comparisons, dedupe aligned, existing_count initialized';

-- ============================================
-- 3. Verificación de integridad
-- ============================================
-- Esto fuerza que las vistas sigan funcionando correctamente
SELECT * FROM shadow_d5_summary LIMIT 0;
SELECT * FROM shadow_d5_divergences LIMIT 0;
