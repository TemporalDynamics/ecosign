-- Fix: protect_document_v2 milestone requeue dedupe must be witness-scoped for incremental signing.
-- Date: 2026-02-18
-- Why:
--   A single document_entity can produce multiple witness hashes (multi-signer incremental flow).
--   If requeue dedupe for protect_document_v2 is only doc+stage, later milestones can be skipped.
--
-- Scope:
--   - Keep awaiting_tsa dedupe doc-scoped.
--   - Make awaiting_anchors / awaiting_artifact dedupe witness-scoped when witness_hash is available.
...
COMMENT ON FUNCTION process_document_entity_events() IS
  'Canonical events listener with witness-scoped dedupe for run_tsa/submit_anchor_*/protect_document_v2 milestones (anchors/artifact) and doc-scoped dedupe for build_artifact/awaiting_tsa.';
