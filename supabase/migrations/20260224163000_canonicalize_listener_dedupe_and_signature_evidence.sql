-- Canonical hardening: close compat_direct paths by extending listener authority.
-- Date: 2026-02-24
-- Changes:
-- 1) protect_document_v2 awaiting_tsa dedupe becomes witness-scoped.
-- 2) listener can enqueue generate_signature_evidence from canonical event
--    job.generate-signature-evidence.required.
...
BEGIN
  ...
  event_signer_id := COALESCE(last_event->'payload'->>'signer_id', '');
  ...
  IF stage_key IS NOT NULL AND event_id IS NOT NULL THEN
    IF stage_key IN ('awaiting_tsa', 'awaiting_anchors', 'awaiting_artifact') THEN
  ...
  ELSIF event_kind = 'job.generate-signature-evidence.required' THEN
    job_type := 'generate_signature_evidence';
    v_job_dedupe_key := document_entity_id::TEXT || ':generate_signature_evidence:' ||
      COALESCE(NULLIF(event_witness_hash, ''), 'none') || ':' ||
      COALESCE(NULLIF(event_signer_id, ''), 'none');
  END IF;
...
COMMENT ON FUNCTION process_document_entity_events() IS
  'Canonical listener: witness-scoped dedupe for protect_document_v2 milestones (incl. awaiting_tsa) and canonical enqueue for signature evidence jobs.';
