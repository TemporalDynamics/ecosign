-- signer.accessed is an observational event, not an authority state transition.
-- Keep signer status signable (ready_to_sign) and only project first_accessed_at.
...
-- Backfill: re-project all workflows so signers previously materialized as "accessed"
-- return to signable status when appropriate.
DO $$
DECLARE
  r RECORD;
BEGIN
