...
addons AS (
  SELECT wa.workspace_id,
         ...
         COALESCE(SUM(CASE WHEN wa.addon_type = 'supervisor_seats' AND wa.status = 'active'
           THEN wa.quantity ELSE 0 END), 0)::int AS supervisors_addon,
...
overrides AS (
  SELECT wl.workspace_id,
         wl.seats_limit,
         wl.agent_seats_limit,
         wl.supervisor_seats_limit,
         wl.storage_gb_limit,
...
SELECT
  ...
  COALESCE(a.supervisors_addon, 0) AS supervisors_addon,
  ...
  -- Legacy seats_limit remains aligned to agent seats for now.
  COALESCE(
    o.seats_limit,
    CASE
  ...
  COALESCE(
    o.agent_seats_limit,
    CASE
      WHEN cp.agent_seats_limit IS NULL THEN NULL
      ELSE cp.agent_seats_limit + COALESCE(a.seats_addon, 0)
    END
  ) AS agent_seats_limit,
  COALESCE(
    o.supervisor_seats_limit,
    CASE
      WHEN cp.supervisor_seats_limit IS NULL THEN NULL
      ELSE cp.supervisor_seats_limit + COALESCE(a.supervisors_addon, 0)
    END
  ) AS supervisor_seats_limit,
  COALESCE(
    o.storage_gb_limit,
    CASE
